...
module.exports = function (pluginOptions) {
  ...
  arrayedOptions.forEach(customizedOptions => {
    ...
    const walkRuleFunction = (rule) => {
      ...
      const file = rule.source && rule.source.input.file.replace(/\\/g, '/');
  ...
  return {
    ...
    Root(css) {
      ...
      // use media
      const landscapeRoot = new postcss.AtRule({ params: '(min-aspect-ratio: 14/9) and (orientation: landscape)', name: 'media' });
      landscapeRoot.append([...landscapeRules]);
      css.append(landscapeRoot);

      // use class
      // landscapeRules.forEach(rule => rule.selectors = rule.selectors.map(selector => '[data-landscape="true"]' + ' ' + selector));
      // css.append(landscapeRules);
    }
  };
};
...
function getUnit(prop, options) {
  return /^font|line-height/i.test(prop) ? options.fontViewportUnit : options.viewportUnit;
}
